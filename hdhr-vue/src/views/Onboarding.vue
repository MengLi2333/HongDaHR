<template>
  <div id="OnboardingDiv">
    ...
    <el-form
      ref="OnboardingForm"
      id="OnboardingForm"
      :label-position="OnboardingForm.labelPosition"
      :model="emp"
      :rules="rules"
      label-width="100px"
      inline
    >
      ...
      <el-form-item label="最高学历" prop="eduLevel">
        <el-select v-model="emp.eduLevel">
          <el-option
            v-for="item in OnboardingForm.eduction_list"
            :key="item.value"
            :label="item.label"
            :value="item.value"
          >
      ...
      <el-form-item label="民族" prop="nation">
        <el-select v-model="emp.nation">
          <el-option
            v-for="item in OnboardingForm.nation_list"
            :key="item.value"
            :label="item.label"
            :value="item.value"
          >
      ...
      <el-form-item label="政治面貌" prop="party">
        <el-select v-model="emp.party">
          <el-option
            v-for="item in OnboardingForm.poli_list"
            :key="item.value"
            :label="item.label"
            :value="item.value"
          >
      ...
      <el-form-item label="健康状况" prop="health">
        <el-select v-model="emp.health">
          <el-option
            v-for="item in OnboardingForm.health_list"
            :key="item.value"
            :label="item.label"
            :value="item.value"
          >
      ...
      <el-form-item label="部门" prop="deptID">
        <el-select v-model="emp.deptID">
          <el-option
            v-for="item in OnboardingForm.department_list"
            :key="item.value"
            :label="item.label"
            :value="item.value"
          >
      ...
      <el-form-item label="岗位" prop="postID">
        <el-select v-model="emp.postID">
          <el-option
            v-for="item in OnboardingForm.post_list"
            :key="item.value"
            :label="item.label"
            :value="item.value"
          >
      ...
      <el-form-item label="职称" prop="titleID">
        <el-select v-model="emp.titleID">
          <el-option
            v-for="item in OnboardingForm.pro_title_list"
            :key="item.value"
            :label="item.label"
            :value="item.value"
          >
      ...
      <el-form-item label="状态" prop="status">
        <el-select v-model="emp.status">
          <el-option
            v-for="item in OnboardingForm.emp_status_list"
            :key="item.value"
            :label="item.label"
            :value="item.value"
          >
      ...
      <el-form-item style="text-align: center; width: 100%">
        <el-button type="primary" @click="OnSubmit('OnboardingForm')"
          >提交</el-button
        >
      </el-form-item>
    </el-form>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        titleID: [{ required: true, message: "请输入职称", trigger: "blur" }],
        postID: [{ required: true, message: "请输入岗位", trigger: "blur" }],
        idCardCode: [
      ...
      request({
        ...
        .then((res) => {
          ...
          if (res.data.status === 200) {
            ...
            post_list += "]";
            this.OnboardingForm.post_list = eval("(" + post_list + ")");
        ...
        .then((res) => {
          if (res.data.status === 200) {
            ...
            title_list += "]";
            this.OnboardingForm.pro_title_list = eval("(" + title_list + ")");
        ...
        .then((res) => {
          if (res.data.status === 200) {
            ...
            dept_list += "]";
            this.OnboardingForm.department_list = eval("(" + dept_list + ")");
  ...
  methods: {
    OnSubmit(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$alert("员工信息添加成功！", "员工信息添加", {
            confirmButtonText: "确定",
          });
        } else {
          this.$alert("员工信息添加失败！", "员工信息添加", {
            confirmButtonText: "确定",
          });
          return false;
        }
