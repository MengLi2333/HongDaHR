  ...
  <div>
    ...
    >
      <el-form
        ref="CheckForm"
        id="CheckForm"
        :label-position="CheckForm.labelPosition"
        :model="CheckForm"
        label-width="80px"
        inline
      >
        <el-form-item label="姓名" prop="empName">
          <el-input
            v-model="CheckForm.MarkedRow.empName"
        ...
        <el-form-item label="最高学历" prop="eduLevel">
          <el-select
            v-model="CheckForm.MarkedRow.eduLevel"
            :disabled="disableFlag"
          >
            <el-option
              v-for="item in eduction_list"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            >
        ...
        <el-form-item label="性别" prop="gender">
          <el-select
            v-model="CheckForm.MarkedRow.gender"
            :disabled="disableFlag"
          >
            <el-option
        ...
        <el-form-item label="民族" prop="nation">
          <el-select
            v-model="CheckForm.MarkedRow.nation"
            :disabled="disableFlag"
          >
            <el-option
        ...
        <el-form-item label="政治面貌" prop="party">
          <el-select
            v-model="CheckForm.MarkedRow.party"
            :disabled="disableFlag"
          >
            <el-option
              v-for="item in poli_list"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            >
        ...
        <el-form-item label="身份证号" prop="idCardCode;">
          <el-input
            v-model="CheckForm.MarkedRow.idCardCode"
            :disabled="disableFlag"
          ></el-input>
        </el-form-item>
        ...
        <el-form-item label="健康状况" prop="health">
          <el-select
            v-model="CheckForm.MarkedRow.health"
            :disabled="disableFlag"
          >
            <el-option
              v-for="item in health_list"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            >
        ...
        <el-form-item label="籍贯" prop="nativePlace;">
          <el-input
            v-model="CheckForm.MarkedRow.nativePlace"
            :disabled="disableFlag"
          ></el-input>
        </el-form-item>
        ...
        <el-form-item label="户口类型" prop="regType;">
          <el-select
            v-model="CheckForm.MarkedRow.regType"
            :disabled="disableFlag"
          >
            <el-option
              v-for="item in regType_list"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            >
        ...
        <el-form-item label="部门" prop="deptID">
          <el-select
            v-model="CheckForm.MarkedRow.deptID"
            :disabled="disableFlag"
          >
            <el-option
        ...
        <el-form-item label="岗位" prop="postID">
          <el-select
            v-model="CheckForm.MarkedRow.postID"
            :disabled="disableFlag"
          >
            <el-option
        ...
        <el-form-item label="职称" prop="titleID">
          <el-select
            v-model="CheckForm.MarkedRow.titleID"
            :disabled="disableFlag"
          >
            <el-option
              v-for="item in pro_title_list"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            >
        ...
        <el-form-item label="状态" prop="status">
          <el-select
            v-model="CheckForm.MarkedRow.status"
            :disabled="disableFlag"
          >
            <el-option
              v-for="item in emp_status_list"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            >
    ...
    >
      ...
      <el-table-column prop="idCardCode" label="身份证号" width="200">
      </el-table-column>
    ...
    <div style="width: 100px; margin: auto; top: auto">
      <el-button @click="loadEmpData" style="text-align: center"
        >刷新</el-button
      >
    </div>
  </div>
...
export default {
  methods: {
    ...
    OpenCheckDialog(index, state) {
      ...
      this.CheckForm.MarkedRow = this.filteredData[index];
      this.state == "update"
      ...
      request({
        ...
        .then((res) => {
          if (res.data.status === 200) {
            ...
            var title_list = "[";
            for (let index = 0; index < title_data.length; index++) {
              const element = title_data[index];
              title_list +=
                "{value:" +
                element.titleID +
                ",label:\"" +
            ...
            title_list += "]";
            this.pro_title_list = eval("(" + title_list + ")");
        ...
        .then((res) => {
          if (res.data.status === 200) {
            ...
            dept_list += "]";
            this.department_list = eval("(" + dept_list + ")");
  ...
  computed: {
    filteredData: function () {
      var NameKey = this.CheckForm.NameKey;

      if (NameKey) {
        return this.EmpData.filter((e) => e.empName.indexOf(NameKey) !== -1);
      } else {
        return this.EmpData;
      }
    },
